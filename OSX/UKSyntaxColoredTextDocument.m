...
-(BOOL)	loadDataRepresentation: (NSData*)data ofType: (NSString*)aType
{
	...
	if( evt )
	{
		...
		{
			NSData*					paramData = [param data];
			struct SelectionRange   range;
			
			memmove( &range, [paramData bytes], sizeof(range) );
...
-(void) goToRangeFrom: (NSInteger)startCh toChar: (NSInteger)endCh
{
	...
	[status setStringValue: [NSString stringWithFormat: @"Characters %lu to %lu",
...
{
	NSUInteger		startCh = newSelectedCharRange.location +1,
					endCh = newSelectedCharRange.location +newSelectedCharRange.length;
	NSUInteger		lineNo = 1,
					lastLineStart = 0,
					x;
	NSUInteger		startChLine, endChLine;
	unichar			lastBreakChar = 0;
	...
	if( startCh > endCh )   // Insertion mark!
	{
		...
		[status setStringValue: [NSString stringWithFormat: @"char %lu, line %lu (char %lu in document)", (unsigned long)startChLine, (unsigned long)lineNo, (unsigned long)startCh]];
	}
	else					// Selection
	{
		img = [NSImage imageNamed: @"SelectionRange"];
		[status setStringValue: [NSString stringWithFormat: @"char %lu to %lu, line %lu (char %lu to %lu in document)", (unsigned long)startChLine, (unsigned long)endChLine, (unsigned long)lineNo, (unsigned long)startCh, (unsigned long)endCh]];
	}
